...
const Package = ({ data }) => {
  ...
  return (
    <div
      className="flex font-mon my-3"
      onClick={() => navigate(`/main/catalog/${data.id}`)}
    >
      <img
        src={data.image}
        className="object-cover w-[250px] rounded-tl-[15px] rounded-bl-[15px] "
        alt=""
      />
      <div className="flex justify-around border px-5 py-3 flex-col">
        <div className="flex justify-between ">
          <h2 className="text-text text-[18px] font-[600]">{data.title}</h2>
        </div>
        <div className="mt-3">
          <p className="text-[14px] font-[600] text-[#1B2126]">Описание</p>
          <p className="text-[14px] leading-4 font-[400] text-[#8D9093] mt-1">
            {data.description.slice(0, 90)}...
          </p>
        </div>
        <div className="flex gap-[10px] mt-4 items-end">
          {data.includes.map((item) => {
            return (
              <div key={item.id} className="flex flex-col items-center">
                <img src={item.icon} alt="" />
                <p className="font-[500] text-[13px] ">{item.title}</p>
              </div>
            );
          })}
        </div>
      </div>
      <div className="border rounded-tr-[15px] rounded-br-[15px] flex flex-col justify-around p-3 flex-shrink-0 w-[200px]">
        <div className="flex items-center">
          <p className="text-text text-[24px] font-[700]">{data.price}</p>
          <del className="text-[#BBBCBE] ml-2">${data.discount}</del>
        </div>
        <div className="">
          <p className="text-[#8D9093] text-[14px] font-[500] flex items-center">
            <span>
              <svg
                width="18"
                height="14"
          ...
          <p className="text-[#8D9093] flex items-center mt-1 text-[14px] font-[500]">
            <span>
              <svg
                width="20"
                height="15"
        ...
        <button className="text-white bg-text p-[10px] rounded-[10px]">
          Подробнее
        </button>
      </div>
    </div>
